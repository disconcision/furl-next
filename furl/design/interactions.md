...
Start with row insertion, explicit row selection, row movement, and reference creation. Use explicit **Rows** and **Connect** tools, with held **Option/Alt** activating the relevant tool temporarily while pointing. Double-click enters a cell when Rows has pointer primacy. Keep a separate **Refactor / Refine / Free edit** policy: gesture targeting and admissibility of structural edits are independent decisions. The study leaves ordinary typing unrestricted; Shape and Inspect remain proposed tool families.
...
The icon sketches use an I-beam, stacked rows with arrows, a loose cable, opposing scope brackets, and an eye. Every inventory action has a tool-family assignment. Shared Undo and motion settings are infrastructure; Copy is a variant, not a new brush. Shape separates comb gestures from row/reference drags because their hit targets and lexical consequences differ. These are hypotheses to test; the last two icons do not pretend to activate unimplemented tools.
...
For both pointer deletions, the canvas is the code viewport rectangle including padding, excluding toolbar/status. Leaving it shows a colored perimeter and deletion/refusal status; returning restores the candidate. Keep source intact at pickup, mutate only on release, and record exactly one history entry. Escape, an in-canvas reference miss, pointer cancellation, blur, and tool/policy changes must never consume the source. Undo restores row contents or occurrence binder IDs, and cancels any return animation. This rectangle is provisional; nested/editor-wide canvases will require a more explicit ownership model.
...
2. **Structure layer and row movement.** Whole-row pointer targets, double-click cell activation, keyboard pickup, refactor/refine/free policy, cancellation, and dependency feedback. Test normal single-click editing, row dragging across every column with Rows active, text selection within an active cell, value activation, held-modifier release, blur, stale-source cancellation, and independent/dependent reorder cases.
