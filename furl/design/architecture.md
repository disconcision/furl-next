...
Idle hover/focus connectors appear only in Move and Copy, including temporary Option/Alt activation of Move. Entering or leaving that mode updates a stationary pointer immediately; the hover preference remains independent of gesture and landing feedback.

Refused row moves and pulls beyond a let block's first/last slot give elastic visual resistance: pointer displacement approaches a limit of 0.4 row heights (8.8px on the current grid), using the existing position spring. The last permitted slot remains the layout anchor and neighboring rows never preview an illegal move. Returning to a permitted candidate resumes ordinary Slot/Float movement; releasing a refused move returns to the original source position. Keyboard refusals give one short directional nudge. Reduced motion omits these displacements. Returning to the original slot creates no source edit or Undo entry.

Gesture explanations overlay the retained cursor details inside the existing bordered inspector, leaving the problem counts and footer geometry in place. They remain while a gesture needs them, then clear 2.4 seconds after a refusal is released (or immediately on a new interaction). Zen displays the same temporary message as a floating footer. `test-row-resistance.cjs` checks actual painted resistance and release, semantic refusals, endpoints, permitted recovery, unchanged source/history, footer geometry/restoration, keyboard and reduced motion.
...
## Expression terms, shared tools and parity round (2026-09-05)

The maintained live contract is Edit / Move / Copy. Both structural tools give native syntax handles priority; a one-character neutral buffer separates them from row-whitespace pickup. Double-click temporarily activates native cell editing or values. Existing Rows / Connect labs are retained design records, not current live chrome. See `parity-round.md` for the bounded supported operations.

`FurlGestures` derives whole expression ranges from Hazel `TermData` and individual operator/delimiter handles from `Measured` shards. Highlights span the complete term. Whitespace inside a cell is never a term insertion target. Transfer replaces an entire term, retaining parentheses conservatively for grouping. Moving leaves a native hole; copies recursively allocate new piece IDs. Embedded projector copies are explicitly refused because their private models may also carry source identities. Pattern/type forms are not part of the expression-transfer gesture.

Refine permits a copy into a type-compatible expression hole only when copied variable bindings remain the same (internal binders map to fresh IDs) and no new static errors appear. Free permits whole-term replacement and removal. Self/ancestor moves are rejected. Bound references retain the cable interaction: dragging an existing use to a miss unplugs it in Free, whereas a generic term cancels on an inside-canvas miss and is removed off-canvas. A picked click-away cancels. The offside design record explores replacing disposal with retained pieces.

Row copies remain within their owning let scope. They refresh all IDs and rename copied pattern names plus internal uses, including recursion. Checked copies require simple irrefutable patterns, a conservative total definition, no new static errors and preserved lexical bindings. Fresh row names draw from old Furl and Hazel's `PlaceholderNames` pool, skipping both existing binders and free names. Empty unused named drafts are removable with Backspace; focus stays in the originating column.
