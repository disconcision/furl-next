...
**Zen view:** new visits open Bindings in Zen, showing the Furl wordmark, powered-by-Hazel link and program. Press **F9** or the corner-brackets icon to switch views; your choice is remembered on reload, separately for each local or deployed origin. The program stays anchored near the upper left with margins of about 20 character widths (reduced on small screens). Move the pointer to the top edge to reveal the existing tool icons and exit button; they hide again when you move away. **Shift+F9** reveals and focuses the tools by keyboard, and Escape dismisses that dock. Long programs scroll inside the available space. Native editing, selections, values, gestures and Undo remain available.
